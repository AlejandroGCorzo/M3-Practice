...
module.exports = {
  ...
  listCats: function (age, name) {
    ...
    if (name) return cats.find((el) => el.name === name);
  ...
  increaseAccesoryPopularity: function (accessoryId) {
    ...
    let howManyCatsWearsTheSameShit = cats.reduce(
      (acc, el) =>
        el.accessories.some((el) => el.id === accessoryId) ? acc + 1 : acc + 0,
      0
    );
    // comparación(retorna booleano) ? si es true hace esto : si es falso hace esto;

    // if (el.accessories.some((el) => el.id === accessoryId)) return acc + 1;
    // return acc + 0;

    // howManyCatsWearsTheSameShit === 2 ? (accesory.popularity = 'average') : 1;

    // howManyCatsWearsTheSameShit === 3 ? (accesory.popularity = 'high') : 1;

    if (howManyCatsWearsTheSameShit === 2) {
      accesory.popularity = 'average';
      return true;
    }

    if (howManyCatsWearsTheSameShit === 3) {
