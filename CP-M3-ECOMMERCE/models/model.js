...
module.exports = {
  ...
  listProducts: function (category, fullName) {
    ...
    if (category && !categories.some((el) => el === category))
      throw Error('La categoría no existe');
    let index = categories.indexOf(category) + 1;
    if (category && fullName)
      return products
        .filter((el) => el.categoryId === index)
        .map((el) => {
          return `${el.brand} ${el.name}`;
        });
    if (category) return products.filter((el) => el.categoryId === index);
    return products;
  },
  ...
  addReview: function (name, stars, text, user) {
    ...
    let product = products.find((el) => el.name === name);
    ...
    product.rating =
      product.reviews.reduce((acc, el) => acc + el.stars, 0) /
  ...
  getRating: function (name) {
    ...
    let product = products.find((el) => el.name === name);
    if (!product && name) throw Error('Producto no encontrado');
    if (name) return product.rating;
    return products
